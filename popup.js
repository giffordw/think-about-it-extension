    ...
    debugLink.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (tab.url.startsWith("chrome://") || 
          ...
          statusElement.innerHTML = "Error: Content scripts cannot run on browser pages";
          return;
        }
        ...
        const injectScript = () => {
          return new Promise((resolve, reject) => {
            statusElement.innerHTML = "Injecting content script...";
        ...
        // First try to send message to see if content script is already there
        try {
          ...
          try {
            await injectScript();
            statusElement.innerHTML = "Content script injected, waiting for initialization...";
            
            // Give the script time to initialize
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // Try to communicate with the injected script
            try {
              await sendDebugMessage();
              statusElement.innerHTML = "Debug successful! Check the webpage for debug overlay.";
            } catch (msgError) {
              statusElement.innerHTML = "Error: Content script was injected but didn't initialize properly.";
              console.error("Content script message error after injection:", msgError);
            }
          } catch (injectionError) {
            statusElement.innerHTML = "Error: Could not inject content script. The site may have content security restrictions.";
